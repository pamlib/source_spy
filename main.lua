...
function OldHighlight:init(frame)
...
function OldHighlight:setRaw(raw)
    raw = raw .. "\n"
    tableContents = {}
    local line = 1
    for i = 1, #raw do
...
function OldHighlight:getString()
    local result = ""
    for _, char in pairs(tableContents) do
...
function OldHighlight:getTable()
    return tableContents
end
function OldHighlight:getSize()
    return #tableContents
end
function OldHighlight:getLine(line)
    local currentline = 0
...
function OldHighlight:setLine(line, text)
...
function OldHighlight:insertLine(line, text)
    if #tableContents and line >= tableContents[#tableContents].Line then
        OldHighlight:setLine(line, text)
    elseif not #tableContents then
        return
    end
    local str = OldHighlight:getRaw()
